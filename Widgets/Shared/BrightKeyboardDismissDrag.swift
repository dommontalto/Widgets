...
extension View {
    ...
    // with the finger, the way dragging the thread already does. The probe is
    // only mounted while the keyboard is up: a platform view left in place
    // would vanish ahead of the view's removal transition.
    func brightKeyboardDismissDrag(isActive: Bool) -> some View {
        background {
            if isActive {
                KeyboardDismissProbe()
            }
        }
    }
}
...
private final class TrackingScrollView: UIScrollView, UIGestureRecognizerDelegate {
    ...
    override func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        guard let region, let pan = gestureRecognizer as? UIPanGestureRecognizer else { return false }
