...
struct BrightScreenEdgeBeam: View {
    ...
    var colorVariant: BeamColorVariant = .defaultCyan
    var size: BeamSize = .md
    // Pins a lap time; nil keeps the shared pace from `BorderBeamPace`.
    var duration: Double?
    ...
    private func beamLayer(in screen: CGSize) -> some View {
        BorderBeam(
            ...
            theme: .dark,
            duration: duration,
            // The canvas is `renderScale` times smaller than the screen, so
            // the pace is scaled down to match what the eye sees.
            speed: BorderBeamPace.pointsPerSecond / renderScale,
            active: isActive,
