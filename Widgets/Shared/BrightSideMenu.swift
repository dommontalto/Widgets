...
//
// The menu pane is only in the stack while it's showing or on its way. A pane
// laid out beside the content while the keyboard is up is taller than the
// keyboard leaves room for, the stack grows to fit it, and anything the content
// hangs off its bottom edge — a chat's input card — ends up under the keyboard.
struct BrightSideMenu<MenuContent: View, Content: View>: View {
    var isEnabled: Bool = true
    var canOpenBySwipe: Bool = true
    var sideBarWidth: CGFloat = 280
    // What sits behind the menu and behind the content. Lighthouse passes its
    // glass so the app still shows through; the home screen keeps the solids.
    var menuBackground: AnyView = AnyView(Color.defaultSheetBackground)
    var contentBackground: AnyView = AnyView(Color.defaultBackground)
