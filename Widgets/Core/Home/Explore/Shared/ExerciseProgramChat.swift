...
nonisolated struct ExerciseProgramChatMessage<Payload>: Identifiable, Equatable {
    enum Kind: Equatable {
        case user
        case assistant
...
nonisolated struct ExerciseProgramChatExample {
    let symbol: String
    let prompt: String
...
struct ExerciseProgramChat<Payload, Response: View, ModelPicker: View>: View {
    let messages: [ExerciseProgramChatMessage<Payload>]
    let isThinking: Bool
    let isBusy: Bool
    var isTyping: FocusState<Bool>.Binding
    var emptyState: ExerciseProgramChatEmptyState?
    var suggestions: ExerciseProgramChatSuggestions?
    ...
    @ViewBuilder
    private func row(for message: ExerciseProgramChatMessage<Payload>) -> some View {
        switch message.kind {
        case .user:
            userBubble(message)
        case .assistant:
    ...
    // background.
    private func userBubble(_ message: ExerciseProgramChatMessage<Payload>) -> some View {
    ...
    private func sentBubble(_ text: String) -> some View {
        BrightText(text, size: Constants.bubbleTextSize, color: bubbleTint)
            .lineSpacing(.lineSpacingMedium)
    ...
    private func failureRow(_ message: ExerciseProgramChatMessage<Payload>) -> some View {
    ...
    private func suggestionChips(_ suggestions: ExerciseProgramChatSuggestions) -> some View {
        HStack(spacing: .spacing0x) {
    ...
    private func send() {
        ...
        flight.begin(
            ...
            textSize: Constants.bubbleTextSize,
            horizontalPadding: Constants.bubblePadding
        )
        onSend(text)
...
private enum Constants {
    ...
    static let bubbleTextSize = FontSizes.body1
    static let bubblePadding: CGFloat = .spacing3x
...
extension ExerciseProgramChat where ModelPicker == EmptyView {
    init(
        messages: [ExerciseProgramChatMessage<Payload>],
        isThinking: Bool,
        isBusy: Bool,
        isTyping: FocusState<Bool>.Binding,
        emptyState: ExerciseProgramChatEmptyState? = nil,
        suggestions: ExerciseProgramChatSuggestions? = nil,
